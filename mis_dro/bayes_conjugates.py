...
def get_posterior_params(posterior: str, data: np.ndarray, prior_params: tuple) -> tuple:
...
def derive_analytical_posterior_params(posterior: str, posterior_params: tuple) -> np.ndarray:
    ...
    if posterior == "normal_gamma":
        ...
        raise NotImplementedError(f"We haven't derived an analytical posterior expression for a '{posterior}' posterior")

def sample_posterior(posterior: str, posterior_params: tuple, num_posterior_samples: int, generator: Optional[np.random.Generator] = None) -> np.ndarray:
    """Sample from the posterior"""
    if posterior == "gamma":
        alpha_post, beta_post = posterior_params
        return sp.stats.gamma.rvs(a=alpha_post, scale=1 / beta_post, size=num_posterior_samples, random_state=generator)
    if posterior == "normal_gamma":
        mu_posterior, kappa_posterior, alpha_posterior, beta_posterior = posterior_params
        return normal_gamma_rvs(num_posterior_samples, mu_posterior, kappa_posterior, alpha_posterior, beta_posterior, generator=generator)
    else:
        raise NotImplementedError(f"Posterior '{posterior}' is not implemented")  
