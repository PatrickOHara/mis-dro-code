...
def empirical_DRO_Gotoh_2(x, epsilon, valid_set):
    ...
    cons=({'type': 'eq','fun': lambda q: np.sum(q) - 1},
          {'type': 'ineq','fun': lambda q: epsilon - np.sum([q[index] * (np.log(q[index]) - np.log(empirical_p[valid_element])) for index, valid_element in enumerate(valid_set) if q[index] > 0])}
         )
    res = optimize.minimize(empirical_DRO_Gotoh_1, initial_point, bounds = bnds, constraints = cons, args = (x, epsilon, valid_set))
    optimal_q = res.x
    optimal_obj = - res.fun
    if np.isnan(optimal_obj):
...
def main_empirical_DRO_Gotoh(epsilon):
    ...
    res = optimize.minimize(empirical_DRO_Gotoh_2, initial_point, bounds = bnds, args = (epsilon, valid_set))
    optimal_x = res.x
    return optimal_x
...
def main_Bayesian_DRO_epsilon1():
    ...
    for j in range(number_iteration_theta):
        epsilon_1[j] = np.sum([empirical_p[i] * np.log(empirical_p[i] / theta[j, i]) for i in range(len(xi_space)) if empirical_p[i] > 0])
    res = optimize.minimize(Bayesian_DRO_3_epsilon1, initial_point, bounds = bnds, args = (epsilon_1))
    optimal_x = res.x
...
def main_Bayesian_DRO_epsilon2():
    ...
    for j in range(number_iteration_theta):
        epsilon_2[j] = np.sum([empirical_p[i] * np.log(empirical_p[i] / theta[j, i]) for i in range(len(xi_space)) if empirical_p[i] > 0]) / 2
    res = optimize.minimize(Bayesian_DRO_3_epsilon2, initial_point, bounds = bnds, args = (epsilon_2))
    optimal_x = res.x
...
def calculate_epsilon3(empirical_x, theta_index):
    ...
    m.addConstr(gp.quicksum(temp_q[i] * temp_nabla[i] for i in range(len(xi_space))) == 0)
    m.optimize()
    initial_point = np.array(m.x)

    cons=({'type': 'eq','fun': lambda q: np.sum(q) - 1},
          {'type': 'eq', 'fun': lambda q: np.sum([q[i] * temp_nabla[i] for i in range(len(xi_space))])}
         )
    bnds = [(0, 1)] * len(xi_space)

    def epsilon3_obj(q, theta_index):
        return np.sum([q[i] * np.log(q[i] / theta[theta_index, i]) for i in range(len(xi_space))])
    res = optimize.minimize(epsilon3_obj, initial_point, bounds = bnds, constraints = cons, args = (theta_index), method='SLSQP')
    optimal_q = res.x
    optimal_obj = res.fun
    if np.isnan(optimal_obj):
...
def main_Bayesian_DRO_epsilon3(empirical_x):
    ...
    res = optimize.minimize(Bayesian_DRO_3_epsilon3, initial_point, bounds = bnds, args = (epsilon_3))
    optimal_x = res.x
    return optimal_x, epsilon_3

sol_true = rv_discrete(name="weighted_uniform", values=(xi_space, theta_c)).ppf((b - c) / (h + b))
D_data = 10
replication = 200
...
for k in range(replication):
    ...
    for index, epsilon in enumerate(epsilon_set):
        ...
        obj_empirical_DRO_Gotoh[k, index] = cost(solution_empirical_DRO_Gotoh[k, index], data_eval[k])
